...
uint8_t get_strongest_channel(uint8_t* image)
{
    ...
    if ((mean_rgb_values[0] >= mean_rgb_values[1])
        && (mean_rgb_values[0] >= mean_rgb_values[2]))
    {
        return 0;
...
void processImage(char* original_name, char* filtered_name)
{
    // Check image dimensions
    char* ext = get_filename_ext(original_name);
    int* dims = get_img_dims(original_name);

    printf("%s\n", filtered_name);

    // Load the image
    uint8_t* image = read_image(original_name, *(dims), *(dims + 1), CHANNELS, CHANNELS);
    ...
    write_image(filtered_name, median_image,
                *(dims), *(dims + 1), CHANNELS);
